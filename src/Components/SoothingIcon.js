...
import SoothingOption from './SoothingOption';
...
function SoothingIcon() {
    ...
    const soothingOptiontransition = useTransition(showSoothing, null, {
    ...
    </animated.div>
    {
        ...
        className="soothingModePanel">
            <P5Wrapper sketch={SketchZero} />
        </animated.div>)
    }
    {
        soothingOptiontransition.map(({ item, key, props}) =>
        item && 
        <animated.div 
        key={key} style={props}
        className="soothingOption">
           <SoothingOption
