import React from 'react';

function HelpSchizophrenia() {
    return(
        <div className="dataVizContainer">
            <p className="helpList">
                <span>Name of Service for Schizophrenia</span><br/>
                Description of the help, service and/or Call to action. Description of the help, service and/or Call to action. Description of the help, service and/or Call to action.<br/>
                <a href="https://patternbased.com/" rel="noopener noreferrer" target="_blank" className="helpBtn weburl">Website</a>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn phone">000-0000-0000</a>
                <br/>

                <span>Name of Service</span><br/>
                Description of the help, service and/or Call to action. Description of the help, service and/or Call to action. Description of the help, service and/or Call to action.<br/>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn weburl">24hr Emergency chat</a>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn phone">000-0000-0000</a>
                <br/>

                <span>Name of Service</span><br/>
                Description of the help, service and/or Call to action. Description of the help, service and/or Call to action. Description of the help, service and/or Call to action.<br/>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn weburl">Website</a>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn phone">000-0000-0000</a>
                <br/>

                <span>Name of Service</span><br/>
                Description of the help, service and/or Call to action. Description of the help, service and/or Call to action. Description of the help, service and/or Call to action.<br/>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn weburl">Website</a>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn phone">000-0000-0000</a>
                <br/>

                <span>Name of Service</span><br/>
                Description of the help, service and/or Call to action. Description of the help, service and/or Call to action. Description of the help, service and/or Call to action.<br/>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn weburl">Emergency chat</a>
                <a href="https://patternbased.com/" target="_blank" rel="noopener noreferrer" className="helpBtn phone">000-0000-0000</a>
                <br/>
                
                </p>
        </div>
    );
}

export default HelpSchizophrenia;